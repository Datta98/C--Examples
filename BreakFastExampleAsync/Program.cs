...
namespace BreakFastExampleAsync
{
    class Program
    {
        ...
        private static void AsyncCode()
        {
            Coffee cup = PourCoffee();
            Console.WriteLine("coffee is ready");
            Task<Egg> _fryEggTask = Task.Factory.StartNew<Egg>(new Func<object, Egg>(Program.FryEggsWrapper), 2);
            _fryEggTask.Wait();
            Egg _egg = _fryEggTask.Result;
            Console.WriteLine("eggs are ready");
            Task<Bacon> _fryBaconTask = Task.Factory.StartNew<Bacon>(new Func<object, Bacon>(Program.FryBaconWrapper), 2);
            _fryBaconTask.Wait();
            Bacon _bacon = _fryBaconTask.Result;
            Console.WriteLine("bacon is ready");
            Task<Toast> _toastBreadTask = Task.Factory.StartNew<Toast>(new Func<object, Toast>(Program.ToastBreadWrapper), 2);
        ...
        private static async void PrepareBreakFast()
        {
            ...
            Console.WriteLine("Coffee is ready");
            Egg _egg = await Task.Factory.StartNew<Egg>(new Func<object, Egg>(Program.FryEggsWrapper), 2);
            Console.WriteLine("eggs are ready");
            Bacon _bacon = await Task.Factory.StartNew<Bacon>(new Func<object, Bacon>(Program.FryBaconWrapper), 2);
            Console.WriteLine("bacon is ready");
            Toast _toast = await Task.Factory.StartNew<Toast>(new Func<object, Toast>(Program.ToastBreadWrapper), 2);
            ApplyButter(_toast);
            ApplyJam(_toast);
            Console.WriteLine("Toast is ready");
            Juice juice = PourOJ();
